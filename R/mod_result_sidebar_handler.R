...
mod_result_sidebar_handler_ui <- function(id, plot_sidebar_state, selected_output_path, selected_y_scale, selected_time_range) {
  ns <- NS(id)
  boxSidebar(
    id = ns("plot_sidebar"),
    width = 33,
...
mod_result_sidebar_handler_server <- function(id, r) {
  moduleServer(id, function(input, output, session) {
    ...
    r$plot_settings <- list(
      selected_output_path = output_paths()[1],
      selected_y_scale = "log",
      selected_time_range = c(0, 40000)
    )
